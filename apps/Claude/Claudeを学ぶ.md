1. 曖昧な指示を試してみる
2. まずは同期的に確認しながら仕事を任せる
3. 複数のプロセスを回してみる

## 資料

公式サイト:

- https://code.claude.com/docs/ja
- https://code.claude.com/docs/ja/best-practices

およそ13時間の一貫コース。
https://github.com/luongnv89/claude-howto

https://zenn.dev/heku/books/claude-code-guide

https://www.nxcode.io/ja/resources/news/claude-code-tutorial-beginners-guide-2026
https://uravation.com/media/claude-code-beginners-guide-2026/

わからないことはClaude自身に聞く。

## その他

- 何を聞いてくるか
- 何を自分で決めるか
- どんな順序で作業するか

- 公開されているスキルやエージェントを使ってみる
- 公開されているスキルやエージェントの指示を読む
- プラグインやMCPサーバーを導入する
- アプリやスクリプトを作ってみる

Claudeで困ったことを記事にするのではなくて、スキルにする。例えばClaudeでnpmを使うとキャッシュとかで困ってsandboxに設定が必要だから、スキルで変更する。シンプルなスキルなら一回きりでプロンプトに投げて使ってもいいし。
